...
- **Edge `getUserIdFromAuth` JWT decode:** `supabase/functions/_shared/supabase-admin.ts` now decodes the JWT payload as **base64url** (with `-`/`_` normalization and padding) before `JSON.parse`. Plain `atob` on the middle segment could fail for ES256 user tokens, yielding `null` `sub` and spurious 401s even when the gateway saw a valid authenticated JWT.
