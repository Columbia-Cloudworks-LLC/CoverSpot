...
- **Edge Function 401 on `functions.invoke`:** When the browser Supabase client had not finished loading the session from cookies, `functions.invoke` could send the **anon key** as the `Authorization` bearer. The gateway accepts that JWT (`verify_jwt`), but edge helpers such as `verifyUserAuth` expect a verified user session, so `sync-playlists`, `discover-variants`, and `mutate-playlist` returned 401. Added `getAccessToken()` in `src/lib/supabase/client.ts` (awaits `getSession()`), and pass `Authorization: Bearer <access_token>` from `SyncButton`, `MutationButtons`, and `VariantDiscoveryPanel` before invoking functions; show a session-expired message when no token is available.
- **Edge Functions gateway 401 (production sync):** With `verify_jwt = true`, the edge runtime could reject valid ES256 user JWTs before the function body ran (`execution_id: null`, 401 from the relay). Set `verify_jwt = false` for `refresh-spotify-token`, `sync-playlists`, `discover-variants`, and `mutate-playlist` in `supabase/config.toml`. Replaced decode-only `getUserIdFromAuth` with `verifyUserAuth` in `supabase/functions/_shared/supabase-admin.ts`, which uses `admin.auth.getUser(token)` so the caller is validated server-side before using `user.id`.
- **`sync-playlists` Spotify refresh:** If refresh-token exchange fails, the function no longer returns 401 immediately; it logs a warning and continues once with the stored access token so stale `token_expires_at` metadata does not hard-fail a still-valid token. True expiry still surfaces via Spotify 401 / `needsReauth` on the playlist fetch path.
- **OAuth callback Spotify row:** `src/app/auth/callback/route.ts` upserts `users` whenever `provider_token` is present, not only when both access and refresh tokens are returned. If Spotify omits `provider_refresh_token` on a repeat login, the existing `spotify_refresh_token` in the database is preserved instead of skipping the upsert (which left expired access tokens in place).
- **`SyncButton` error surface:** On `functions.invoke` failure, reads the edge JSON body when available (`error`, `needsReauth`) so users see a specific message (e.g. Spotify re-auth) instead of a generic sync failure.
